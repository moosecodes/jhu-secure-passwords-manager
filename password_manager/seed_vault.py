...
def generate_password(length=16, upper=True, lower=True, digits=True, symbols=True, exclude_ambiguous=True) -> str:
    ...
    required = []
    if upper: required.append(secrets.choice("".join(ch for ch in string.ascii_uppercase if ch not in AMBIG)))
    if lower: required.append(secrets.choice("".join(ch for ch in string.ascii_lowercase if ch not in AMBIG)))
    if digits: required.append(secrets.choice("".join(ch for ch in string.digits if ch not in AMBIG)))
    if symbols: required.append(secrets.choice("".join(ch for ch in "!@#$%^&*-_=+?" if ch not in AMBIG)))
    pw = required + [secrets.choice(alphabet) for _ in range(max(0, length - len(required)))]
